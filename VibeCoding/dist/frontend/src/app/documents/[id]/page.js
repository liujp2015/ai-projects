...
const isWordToken = (token) => /^[a-zA-Z0-9'-]+$/.test(token);
...
function DocumentDetailPage() {
    ...
    const highlightSentenceId = (0, react_1.useMemo)(() => searchParams.get('sentenceId'), [searchParams]);
    const [doc, setDoc] = (0, react_1.useState)(null);
    const [userWords, setUserWords] = (0, react_1.useState)({});
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [error, setError] = (0, react_1.useState)(null);
    const [selectedWord, setSelectedWord] = (0, react_1.useState)(null);
    const [definition, setDefinition] = (0, react_1.useState)(null);
    const [defLoading, setDefLoading] = (0, react_1.useState)(false);
    const [currentSentenceId, setCurrentSentenceId] = (0, react_1.useState)(null);
    const [activeTab, setActiveTab] = (0, react_1.useState)('definition');
    const [scenario, setScenario] = (0, react_1.useState)(SCENARIOS[0].label);
    const [userSentence, setUserSentence] = (0, react_1.useState)('');
    ...
    const [viewTab, setViewTab] = (0, react_1.useState)('read');
    const [translationMode, setTranslationMode] = (0, react_1.useState)('zhEn');
    const [docTranslation, setDocTranslation] = (0, react_1.useState)(null);
    const [translationLoading, setTranslationLoading] = (0, react_1.useState)(false);
    const [translatingMissing, setTranslatingMissing] = (0, react_1.useState)(false);
    const [generatingQuestions, setGeneratingQuestions] = (0, react_1.useState)(false);
    const [translationError, setTranslationError] = (0, react_1.useState)(null);
    const [showAppendPanel, setShowAppendPanel] = (0, react_1.useState)(false);
    ...
    const [testScrambleIndices, setTestScrambleIndices] = (0, react_1.useState)([]);
    const [testChoiceSelection, setTestChoiceSelection] = (0, react_1.useState)(null);
    ...
    const loadTranslation = async () => {
        ...
        try {
            ...
            const data = await (0, api_1.fetchDocumentTranslation)(id);
    ...
    const handleStartTranslate = async () => {
        if (!id || translatingMissing)
            return;
        try {
            setTranslatingMissing(true);
            await (0, api_1.translateAlignRebuild)(id);
            const [docData, wordsData] = await Promise.all([
                (0, api_1.fetchDocument)(id),
                (0, api_1.fetchUserWords)(),
            ]);
            setDoc(docData);
            const wordMap = {};
            wordsData.forEach((w) => {
    ...
    const checkTestAnswer = () => {
        ...
        if (currentQuestion.type === 'SCRAMBLE') {
            ...
            if (!sc || !sc.blanks)
                return;
            const allCorrect = sc.blanks.every((blank) => {
            ...
            if (isCorrect) {
                ...
                const correctAnswers = sc.blanks.map((b) => `[${b.blank_index}] ${b.correct_answer}`).join(', ');
    ...
    const handleAppendText = async () => {
        ...
        try {
            setAppending(true);
            await (0, api_1.appendText)(id, appendTextContent);
            alert('内容追加成功！正在刷新数据...');
    ...
    const handleAppendImages = async (e) => {
        ...
        if (!id || !files || files.length === 0 || appending)
            return;
        try {
            setAppending(true);
            const fileList = Array.from(files);
            await (0, api_1.appendImages)(id, fileList);
    ...
    const readParagraphs = (0, react_1.useMemo)(() => {
        const text = String(doc?.originalText ?? '').trim();
        if (!text)
            return null;
        const paragraphTexts = text.split(/\n\s*\n/).map((p) => p.trim()).filter(Boolean);
        return paragraphTexts.map((pText, pIdx) => {
            const sentenceTexts = pText.match(/[^.!?]+[.!?]+|\s*[^.!?]+$/g)?.map((s) => s.trim()).filter(Boolean) ?? [pText];
            return {
                id: `local-p-${pIdx}`,
    ...
    const handleTextSelection = () => {
        ...
        if (selectedText && selectedText.includes(' ') && selectedText.length < 100) {
            ...
            if (cleanPhrase.length > 1) {
                ...
                setUserSentence('');
                (0, api_1.lookupWord)(cleanPhrase.toLowerCase()).then(def => {
                    setDefinition(def);
                }).catch(err => {
    ...
    const handleWordClick = async (word, sentenceId) => {
        ...
        setUserSentence('');
        try {
            const def = await (0, api_1.lookupWord)(cleanWord);
            setDefinition(def);
        }
        catch (err) {
    ...
    const handleValidate = async (e) => {
        e.preventDefault();
        if (!selectedWord || !userSentence.trim() || validating)
            return;
        setValidating(true);
        try {
            const result = await (0, api_1.validateSentence)(selectedWord, scenario, userSentence);
    ...
    const toggleCollection = async () => {
        ...
        try {
            if (isCollected) {
                await (0, api_1.deleteUserWord)(wordKey);
                const newWords = { ...userWords };
                delete newWords[wordKey];
                setUserWords(newWords);
            }
            else {
                const saved = await (0, api_1.upsertUserWord)(wordKey, 'NEW', currentSentenceId || undefined, definition?.translation, definition?.definitionZh);
    ...
    const getHighlightClass = (word) => {
        ...
        if (status === 'NEW')
            return 'bg-yellow-200 dark:bg-yellow-900/50 border-b-2 border-yellow-400';
        if (status === 'LEARNING')
            return 'bg-blue-100 dark:bg-blue-900/30 border-b-2 border-blue-400';
        if (status === 'MASTERED')
    ...
    if (error || !doc) {
        ...
        <link_1.default href="/documents" className="text-blue-600 hover:underline">Back to Library</link_1.default>
      </div>);
    }
    return (<div className="min-h-screen bg-white flex flex-col md:flex-row">
      ...
      <div className="flex-1 overflow-auto border-r border-gray-100">
        ...
        <main className="max-w-3xl mx-auto px-8 py-12" onMouseUp={handleTextSelection}>
          {viewTab === 'read' ? (<div className="space-y-10">
            {(readParagraphs ?? doc.paragraphs).map((para) => (<div key={para.id} className="paragraph-container">
                <div className="text-[1.15rem] leading-[1.8] text-gray-800 tracking-wide">
                  {para.sentences.map((sent) => (<span key={sent.id} id={`sent-${sent.id}`} className={`group sentence-block inline mr-1 transition-all duration-1000 p-1 rounded ${highlightSentenceId === sent.id
                      ...
                      {tokenize(sent.content).map((token, idx) => {
                        ...
                        return isWord ? (<span key={`${sent.id}-${idx}`} onClick={() => handleWordClick(token, readParagraphs ? '' : sent.id)} className={`cursor-pointer px-0.5 rounded transition-all hover:bg-blue-100 dark:hover:bg-blue-900/40 ${getHighlightClass(token)}`}>
                            {token}
                          </span>) : (<span key={`${sent.id}-${idx}`}>{token}</span>);
                    })}
                      
                      <button onClick={(e) => {
                        e.stopPropagation();
                        playAudio(sent.content);
                    }} className={`ml-1 p-1 rounded-full hover:bg-blue-200 transition-all opacity-0 group-hover:opacity-100 ${playingText === sent.content ? 'opacity-100 bg-blue-100 text-blue-600 animate-pulse' : 'text-gray-400'}`} title="朗读句子">
            ...
            <div className="pt-10 border-t border-gray-100">
              {!showAppendPanel ? (<button onClick={() => setShowAppendPanel(true)} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-3xl text-gray-400 font-medium hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50/30 transition-all flex items-center justify-center gap-2 group">
                  ...
                  追加新内容 (图片或文本)
                </button>) : (<div className="bg-gray-50 rounded-3xl p-6 border border-gray-200 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-gray-900">追加学习内容</h3>
                    <button onClick={() => setShowAppendPanel(false)} className="text-gray-400 hover:text-gray-600">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="relative group">
                        <input type="file" multiple accept="image/*" onChange={handleAppendImages} disabled={appending} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 disabled:cursor-not-allowed"/>
                    ...
                    <div>
                      ...
                      <textarea rows={4} value={appendTextContent} onChange={(e) => setAppendTextContent(e.target.value)} placeholder="在此粘贴新的英文内容..." className="w-full p-4 bg-white border border-gray-200 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all"/>
                      <div className="mt-4 flex justify-end">
                        <button onClick={handleAppendText} disabled={appending || !appendTextContent.trim()} className="px-8 py-2 bg-blue-600 text-white rounded-full text-sm font-bold shadow-md hover:bg-blue-700 transition-all disabled:opacity-50">
              ...
              {!showTest ? (<>
                  ...
                  <div className="space-y-10">
                    ...
                    <div>
                      ...
                      {currentQuestion.type === 'SCRAMBLE' ? (<div className="min-h-[80px] p-6 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 flex flex-wrap gap-2 text-xl">
                          {testScrambleIndices.map((tokenIdx, displayIdx) => (<span key={`${tokenIdx}-${displayIdx}`} onClick={() => setTestScrambleIndices(prev => prev.filter((_, idx) => idx !== displayIdx))} className="bg-white px-3 py-1 rounded-lg shadow-sm cursor-pointer hover:bg-red-50 hover:text-red-500 transition-colors">
                              {currentQuestion.scrambledTokens[tokenIdx]}
                            </span>))}
                          {testScrambleIndices.length === 0 && <span className="text-gray-300">请点击下方单词...</span>}
                        </div>) : currentQuestion.type === 'SENTENCE_COMPLETION' ? (<div className="p-8 bg-blue-50/30 rounded-3xl border border-blue-100/50">
                        ...
                        if (part === '____') {
                            ...
                            return (<div key={i} className="relative inline-block">
                                    <button onClick={(e) => {
                                    e.stopPropagation();
                                    setActiveBlankIndex(activeBlankIndex === blankIndex ? null : blankIndex);
                                }} className={`min-w-[100px] px-4 py-1 border-b-2 transition-all font-bold ${currentAnswer
                                    ? 'border-blue-500 text-blue-600 bg-blue-50/50'
                                    : 'border-gray-300 text-gray-300 hover:border-blue-400'}`}>
                                      {currentAnswer || `(${blankIndex})`}
                                    </button>
                                    {activeBlankIndex === blankIndex && (<div className="absolute top-full left-0 mt-2 z-20 bg-white shadow-2xl rounded-2xl border border-gray-100 p-2 min-w-[160px]" onClick={(e) => e.stopPropagation()}>
                                        {currentQuestion.structuredData?.blanks?.find((b) => b.blank_index === blankIndex)?.options.map((opt) => (<button key={opt} onClick={(e) => {
                                            e.stopPropagation();
                        ...
                        return <span key={i}>{part}</span>;
                          ...
                          {(currentQuestion.blankedEn || currentQuestion.type === 'WORD_MATCHING') && (<div className="p-6 rounded-3xl bg-gray-50 border border-gray-100 text-xl text-gray-700 leading-relaxed">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {currentQuestion.options.map((option, i) => (<button key={i} disabled={!!testResult} onClick={() => setTestChoiceSelection(option)} className={`p-6 rounded-2xl border-2 text-left transition-all font-bold text-lg ${testChoiceSelection === option
                            ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-md scale-[1.02]'
                            : 'border-gray-100 hover:border-blue-200 hover:bg-gray-50/50'}`}>
                                <span className={`inline-block w-10 h-10 rounded-full text-center leading-10 mr-4 text-sm font-black ${testChoiceSelection === option ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
                    ...
                    {!testResult ? (currentQuestion.type === 'SCRAMBLE' && (<div className="flex flex-wrap gap-2">
                          {currentQuestion.scrambledTokens.map((word, tokenIdx) => {
                        const isSelected = testScrambleIndices.includes(tokenIdx);
                        return (<button key={tokenIdx} onClick={() => {
                                if (isSelected) {
                            ...
                            }} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all shadow-sm ${isSelected
                                ? 'bg-blue-100 border-2 border-blue-500 text-blue-700'
                                : 'bg-white border border-gray-200 hover:border-blue-500 hover:text-blue-600'}`}>
                                {word}
                              </button>);
                    })}
                        </div>)) : (<div className={`p-6 rounded-2xl ${testResult.isCorrect ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                        <p className="font-bold mb-1">{testResult.isCorrect ? '回答正确！' : '回答错误'}</p>
                        <p className="text-sm opacity-80">{testResult.message}</p>
                      </div>)}

                    <div className="pt-6 border-t border-gray-100 flex gap-4">
                      {!testResult ? (<button onClick={checkTestAnswer} disabled={(() => {
                        if (currentQuestion.type === 'SCRAMBLE')
                            return testScrambleIndices.length === 0;
      ...
      <aside className={`w-full md:w-[380px] bg-gray-50/50 p-6 transition-all border-t md:border-t-0 ${selectedWord ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 pointer-events-none md:translate-y-0'}`}>
        {selectedWord ? (<div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sticky top-24">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className="text-3xl font-bold text-gray-900">{selectedWord}</h2>
                {definition?.phonetic && (<p className="text-blue-600 font-mono mt-1">{definition.phonetic}</p>)}
                {definition?.translation && (<p className="text-gray-700 mt-2">{definition.translation}</p>)}
              </div>
              <div className="flex gap-2">
                <button onClick={() => setActiveTab(activeTab === 'definition' ? 'practice' : 'definition')} className={`p-3 rounded-full transition-colors ${activeTab === 'practice' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'}`} title="造句练习">
                ...
                <button onClick={toggleCollection} className={`p-3 rounded-full transition-colors ${userWords[selectedWord.toLowerCase()] ? 'bg-yellow-100 text-yellow-600 hover:bg-yellow-200' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`} title={userWords[selectedWord.toLowerCase()] ? 'Remove from Collection' : 'Add to Collection'}>
            ...
            {activeTab === 'definition' ? (defLoading ? (<div className="space-y-3 py-4">
                  <div className="h-4 bg-gray-100 rounded w-3/4 animate-pulse"></div>
                  <div className="h-4 bg-gray-100 rounded w-full animate-pulse"></div>
                  <div className="h-4 bg-gray-100 rounded w-1/2 animate-pulse"></div>
                </div>) : (<div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                  {definition?.definitionZh && (<div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100/50">
                  ...
                  {definition?.meanings?.length ? (definition.meanings.map((m, idx) => (<div key={idx} className="border-t border-gray-50 pt-4 first:border-0 first:pt-0">
                        <span className="text-[10px] uppercase tracking-wider font-bold text-gray-400 bg-gray-50 px-2 py-0.5 rounded">
                          {m.partOfSpeech}
                        </span>
                        <ul className="mt-3 space-y-3">
                          {m.definitions.map((d, dIdx) => (<li key={dIdx} className="text-sm text-gray-700 leading-relaxed">
                              <p className="font-medium">• {d.definition}</p>
                              {d.example && (<p className="mt-1.5 text-gray-500 italic border-l-2 border-gray-100 pl-3">"{d.example}"</p>)}
                            </li>))}
                        </ul>
                      </div>))) : !definition?.definitionZh && (<p className="py-10 text-center text-gray-400 italic">No definitions found for this word.</p>)}
                </div>)) : (<div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
                {!validationResult ? (<form onSubmit={handleValidate} className="space-y-6">
                    <div>
                      <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 block">选择使用场景</label>
                      <div className="grid grid-cols-3 gap-2">
                        {SCENARIOS.map((s) => (<button key={s.id} type="button" onClick={() => setScenario(s.label)} className={`flex flex-col items-center gap-1 p-2 rounded-xl border transition-all ${scenario === s.label ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-100 bg-white text-gray-500 hover:border-blue-200'}`}>
                    ...
                    <div>
                      ...
                      <textarea required rows={4} value={userSentence} onChange={(e) => setUserSentence(e.target.value)} className="w-full p-4 bg-gray-50 border-0 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none" placeholder={`使用 "${selectedWord}" 造一个关于 ${scenario} 的句子...`}/>
                    </div>

                    <button type="submit" disabled={validating || !userSentence.trim()} className="w-full py-4 bg-black dark:bg-white text-white dark:text-black rounded-2xl font-bold text-sm shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50">
                      {validating ? (<span className="flex items-center justify-center gap-2">
            ...
            <button onClick={() => setSelectedWord(null)} className="mt-8 w-full py-2.5 text-sm text-gray-500 hover:text-gray-700 font-medium border-t border-gray-100">
              Close
            </button>
          </div>) : (<div className="h-full flex flex-col items-center justify-center text-center px-10">
