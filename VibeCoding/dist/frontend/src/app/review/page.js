...
function ReviewPage() {
    ...
    (0, react_1.useEffect)(() => {
        const loadQueue = async () => {
            try {
                setLoading(true);
                const data = await (0, api_1.fetchReviewQueue)();
                setQueue(data);
            }
            catch (err) {
                setError('加载复习队列失败');
    ...
    const handleScore = async (quality) => {
        ...
        try {
            setSaving(true);
            await (0, api_1.submitReview)(currentWord.word, quality);
            setIsFlipped(false);
    ...
    if (loading)
        return <div className="min-h-screen flex items-center justify-center">加载复习任务中...</div>;
    if (error)
        return <div className="min-h-screen flex flex-col items-center justify-center gap-4">
    <p className="text-red-500">{error}</p>
    <link_1.default href="/documents" className="text-blue-600 underline">返回首页</link_1.default>
  </div>;
    if (queue.length === 0) {
        ...
        <link_1.default href="/documents" className="px-6 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition-colors">
          回到我的书库
        </link_1.default>
      </div>);
    }
    const word = queue[currentIndex];
    return (<div className="min-h-screen bg-gray-50 flex flex-col items-center p-8">
      <header className="w-full max-w-2xl flex justify-between items-center mb-12">
        <link_1.default href="/documents" className="text-gray-500 hover:text-gray-700 font-medium">退出复习</link_1.default>
      ...
      <main className="w-full max-w-2xl flex-1 flex flex-col items-center justify-center">
        ...
        <div onClick={() => !isFlipped && setIsFlipped(true)} className={`w-full min-h-[400px] bg-white rounded-3xl shadow-xl border border-gray-100 p-12 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-500 transform ${isFlipped ? '' : 'hover:scale-[1.02]'}`}>
          ...
          {!isFlipped ? (<p className="text-gray-400 animate-pulse mt-8">点击卡片查看释义</p>) : (<div className="w-full border-t border-gray-100 pt-8 mt-8 space-y-6">
              <div>
                <p className="text-2xl font-bold text-blue-600 mb-2">{word.translation}</p>
                {word.definition && (<p className="text-gray-600 text-sm leading-relaxed whitespace-pre-wrap mb-6">{word.definition}</p>)}
                
                {word.sourceSentence && (<div className="mt-6 pt-6 border-t border-gray-50 text-left">
                    ...
                    <p className="text-gray-700 italic leading-relaxed">
                      "{word.sourceSentence.content.split(new RegExp(`(${word.word})`, 'gi')).map((part, i) => part.toLowerCase() === word.word.toLowerCase()
                    ? <span key={i} className="bg-yellow-100 font-bold px-1 rounded">{part}</span>
                    : part)}"
                    </p>
                    <div className="mt-4">
                      <link_1.default href={`/documents/${word.sourceSentence.paragraph.document.id}?sentenceId=${word.sourceSentence.id}`} className="text-xs text-blue-500 hover:underline flex items-center gap-1">
        ...
        <div className={`mt-12 w-full grid grid-cols-3 gap-4 transition-all duration-300 ${isFlipped ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          {SCORES.map((s) => (<button key={s.value} onClick={() => handleScore(s.value)} disabled={submitting} className={`py-4 rounded-2xl text-white font-bold text-lg shadow-lg transition-transform active:scale-95 ${s.color}`}>
              {s.label}
            </button>))}
        </div>
      </main>
