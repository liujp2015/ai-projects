...
export class AIService {
  ...
  async validateSentence(word: string, scenario: string, sentence: string) {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful and professional English teacher who provides feedback in JSON format.',
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.3,
  ...
  async translateEnglishToChinese(sentences: string[]): Promise<string[]> {
    const config = getDeepSeekConfig();
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a professional translator. Translate English to Chinese and output only a JSON array of strings.',
          },
          { role: 'user', content: prompt },
        ],
        temperature: 0.2,
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a professional bilingual editor. Output strictly formatted JSON only.',
          },
          { role: 'user', content: prompt },
        ],
        temperature: 0.2,
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an educational content creator. Generate English exercise questions in strict JSON format.',
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.3,
...
`;
   ...
   */
  async extractWordsFromSentences(sentences: string[]): Promise<Array<{ word: string; partOfSpeech: string; translation: string; sentence: string }>> {
    const config = getDeepSeekConfig();
    if (!config.apiKey) throw new Error('DeepSeek API Key not configured');
