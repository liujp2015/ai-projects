...
function SceneBuilderContent() {
    ...
    const [savedSentences, setSavedSentences] = (0, react_1.useState)([]);
    const [savingSentence, setSavingSentence] = (0, react_1.useState)(null);
    const [nextSuggestion, setNextSuggestion] = (0, react_1.useState)(null);
    const [loadingNext, setLoadingNext] = (0, react_1.useState)(false);
    const [evaluating, setEvaluating] = (0, react_1.useState)(false);
    const [evaluation, setEvaluation] = (0, react_1.useState)(null);
    const [rightTab, setRightTab] = (0, react_1.useState)('analysis');
    const sentence = (0, react_1.useMemo)(() => buildSentence(selected), [selected]);
    const requiredWord = (lexicon?.requiredWord || word).trim();
    const hasRequiredWord = !requiredWord ||
        sentence.toLowerCase().includes(requiredWord.toLowerCase());
    const needsCorePhrase = !!requiredWord;
    const coreChosen = !!selected.object;
    const stepText = (0, react_1.useMemo)(() => {
        if (!lexicon)
            return '先生成词库';
        if (needsCorePhrase && !coreChosen)
            return 'Step 1：先选一个「高频搭配」（包含目标词）';
        if (!selected.subject)
            return 'Step 2：选择主语';
        if (!selected.verb)
            return 'Step 3：选择动作/动词短语';
        if (!selected.modifier)
            return 'Step 4：选择修饰语（可选）';
    ...
    const coreTokens = lexicon?.corePhrases ?? [];
    const tabDisabled = {
    ...
    const orderedSelectedTexts = (0, react_1.useMemo)(() => {
        const parts = [
            selected.object?.text,
            selected.subject?.text,
            selected.verb?.text,
    ...
    const orderedSelectedTokens = (0, react_1.useMemo)(() => {
        const parts = [];
        if (selected.object) {
            parts.push({ category: needsCorePhrase ? 'core' : 'object', id: selected.object.id, text: selected.object.text });
        }
        if (selected.subject)
            parts.push({ category: 'subject', id: selected.subject.id, text: selected.subject.text });
        if (selected.verb)
            parts.push({ category: 'verb', id: selected.verb.id, text: selected.verb.text });
        if (selected.modifier)
            parts.push({ category: 'modifier', id: selected.modifier.id, text: selected.modifier.text });
        return parts;
        ...
        const run = async () => {
            try {
                ...
                if (normalized.nextCategory !== 'done') {
                    const suggestedTab = normalized.nextCategory;
                    if (!tabDisabled[suggestedTab]) {
    ...
    const recommendedIdSet = (0, react_1.useMemo)(() => {
        return new Set(nextSuggestion?.recommendedIds ?? []);
    }, [nextSuggestion?.recommendedIds]);
    const recommendedReasonById = (0, react_1.useMemo)(() => {
        const map = new Map();
    ...
    const recommendedTokens = (0, react_1.useMemo)(() => {
        if (!lexicon || !nextSuggestion || nextSuggestion.nextCategory === 'done')
            return [];
    ...
    const refreshSaved = async (w, sc) => {
        ...
        if (Array.isArray(data)) {
            setSavedSentences(data.map((x) => ({
                id: String(x?.id ?? ''),
                word: String(x?.word ?? ''),
                scene: String(x?.scene ?? ''),
                sentence: String(x?.sentence ?? ''),
                source: String(x?.source ?? 'USER').toUpperCase() ?? 'USER',
                createdAt: String(x?.createdAt ?? ''),
            })).filter((x) => x.id && x.sentence));
        }
    };
    (0, react_1.useEffect)(() => {
        if (!lexicon)
    ...
    const saveSentenceToDb = async (s, source) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <header className="mb-6 flex flex-col gap-3">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
            ...
            {requiredWord && (<div className="flex items-center gap-2">
        ...
        <main className="rounded-3xl bg-white/95 shadow-2xl border border-slate-200 p-6 lg:p-8 space-y-6">
          ...
          <section className="flex flex-col lg:flex-row lg:items-center gap-3">
            <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="flex-1 flex flex-col gap-1">
                ...
                <input className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/60 focus:border-blue-500 bg-slate-50" value={word} onChange={(e) => setWord(e.target.value)} placeholder="例如：luggage（从生词本自动带入）"/>
              </div>
              <div className="flex-1 flex flex-col gap-1">
                <label className="text-xs font-semibold text-slate-500">练习场景</label>
                <input className="border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/60 focus:border-blue-500 bg-slate-50" value={scene} onChange={(e) => setScene(e.target.value)} placeholder="例如：机场值机、医院看病、咖啡厅点单…"/>
              </div>
            </div>
            <button onClick={loadScene} disabled={loadingScene} className="inline-flex items-center justify-center px-4 py-2 rounded-2xl bg-blue-600 text-white text-sm font-semibold shadow-md hover:bg-blue-700 disabled:opacity-50 disabled:shadow-none transition">
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1.1fr)]">
              ...
              <section className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 flex flex-col gap-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="inline-flex items-center gap-2 text-[11px] text-slate-500">
                    ...
                    {nextSuggestion && nextSuggestion.nextCategory !== 'done' && (<span className="inline-flex items-center gap-1">
            ...
            return (<button key={cat} onClick={() => onRemoveToken(cat)} className={`px-2 py-1 rounded-full text-xs font-medium ${color}`}>
                          {token.text} ✕
                        </button>);
        })}
                  {!selected.subject && !selected.verb && !selected.object && !selected.modifier && (<span className="text-xs text-slate-400">
                      先从下方词块中点选，句子会自动拼在这里…
                    </span>)}
                </div>
                ...
                {!!requiredWord && (<div className={`text-xs mt-1 ${hasRequiredWord ? 'text-emerald-700' : 'text-rose-700'}`}>
                    目标单词：<b>{requiredWord}</b>（{hasRequiredWord ? '已包含' : '未包含'}）
                  </div>)}

                <div className="mt-2 flex flex-wrap gap-2">
                  <button onClick={onEvaluate} disabled={!sentence.trim() || evaluating || !hasRequiredWord} className="px-3 py-1.5 rounded-xl bg-emerald-600 text-white text-xs font-semibold disabled:opacity-50">
                    {evaluating ? '分析中…' : '开始研究'}
                  </button>
                  <button onClick={saveCurrentSentence} disabled={!sentence.trim() || !hasRequiredWord} className="px-3 py-1.5 rounded-xl bg-slate-900 text-white text-xs font-semibold disabled:opacity-50">
                    保存为一句
                  </button>
                  <button onClick={newSentenceSameLexicon} disabled={!selected.object} className="px-3 py-1.5 rounded-xl border border-slate-300 text-slate-700 text-xs font-semibold disabled:opacity-40 bg-white">
                    再来一句（保留核心搭配）
                  </button>
                </div>
              </section>
              ...
              {!!requiredWord && lexicon && (<section className="rounded-2xl border border-slate-200 bg-white px-4 py-3 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {coreTokens.length > 0 ? (coreTokens.map((t) => (<button key={t.id} onClick={() => onPickToken('object', t)} className={`px-2 py-1 rounded-full text-xs border ${selected.object?.id === t.id
                    ? 'border-blue-500 bg-blue-50 text-blue-700'
                    : recommendedIdSet.has(t.id)
                        ? 'border-amber-400 bg-amber-50 text-amber-900'
                        : 'border-slate-200 bg-slate-50 text-slate-800 hover:border-blue-300'}`}>
                          {t.text}
                        </button>))) : (<div className="text-sm text-slate-500">
                        暂无核心短语，你可以在下方「核心短语/宾语」里直接选带有目标词的短语。
                      </div>)}
                  </div>
                  {lexicon.suggestedSentences?.length ? (<div className="mt-1 text-xs text-slate-600 space-y-1.5">
                      <div className="font-medium">同场景多句参考（都包含目标词）：</div>
                      <ul className="space-y-1">
                        {lexicon.suggestedSentences.map((s, i) => (<li key={i} className="flex items-start justify-between gap-2">
                            <span className="flex-1">{s}</span>
                            <button className="shrink-0 px-2 py-0.5 rounded-full border text-[10px] text-slate-700 hover:bg-slate-50 disabled:opacity-50" onClick={() => saveSentenceToDb(s, 'SUGGESTED')} disabled={savingSentence === s} title="保存到该生词的句子库">
              ...
              {lexicon && (<section className="rounded-2xl border border-slate-200 bg-white px-4 py-3 flex flex-col gap-3">
                  <div className="flex gap-2 mb-1">
                    {['subjects', 'verbs', 'objects', 'modifiers'].map((k) => (<button key={k} onClick={() => setActiveTab(k)} disabled={tabDisabled[k]} className={`px-3 py-1.5 rounded-full text-xs font-semibold ${tabDisabled[k]
                  ...
                  {recommendedTokens.length > 0 && (<div className="mb-1">
                      <div className="text-xs font-semibold text-amber-700 mb-1">
                        推荐（点一下就行）
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {recommendedTokens.map((t) => (<button key={t.id} onClick={() => {
                        const cat = nextSuggestion?.nextCategory === 'subjects'
                            ? 'subject'
                            : nextSuggestion?.nextCategory === 'verbs'
                                ...
                                : nextSuggestion?.nextCategory === 'objects'
                                    ...
                                    : 'modifier';
                        onPickToken(cat, t);
                    }} title={recommendedReasonById.get(t.id) || ''} className="px-2 py-1 rounded-full text-xs border border-amber-400 bg-amber-50 text-amber-900 hover:bg-amber-100">
                            <span className="font-medium">{t.text}</span>
                            {recommendedReasonById.get(t.id) ? (<span className="ml-2 text-[10px] text-amber-700">
                ...
                const color = activeTab === 'subjects'
                    ...
                    : activeTab === 'verbs'
                        ...
                        : activeTab === 'objects'
                            ? 'bg-orange-100 text-orange-800'
                            : 'bg-slate-100 text-slate-800';
                return (<button key={t.id} onClick={() => onPickToken(cat, t)} className={`px-2 py-1 rounded-full text-xs ${color} ${recommendedIdSet.has(t.id) ? 'ring-2 ring-amber-400' : ''}`} title={recommendedReasonById.get(t.id) || ''}>
                          {t.text}
                        </button>);
            })}
              ...
              <section className="rounded-2xl border border-slate-200 bg-white px-4 py-3 space-y-3">
                <div className="flex items-center justify-between mb-1">
                  <div className="inline-flex rounded-full bg-slate-100 p-1 text-xs">
                    <button onClick={() => setRightTab('analysis')} className={`px-3 py-1 rounded-full font-semibold ${rightTab === 'analysis'
            ? 'bg-white text-slate-900 shadow'
            : 'text-slate-500'}`}>
                      研究结果
                    </button>
                    <button onClick={() => setRightTab('saved')} className={`px-3 py-1 rounded-full font-semibold ${rightTab === 'saved'
            ? 'bg-white text-slate-900 shadow'
            : 'text-slate-500'}`}>
                      已保存句子
                    </button>
                  </div>
                  {rightTab === 'saved' && (<div className="text-[11px] text-slate-500">
                      共 {savedSentences.length} 句
                    </div>)}
                </div>

                {rightTab === 'analysis' ? (evaluation ? (<div className="flex flex-col gap-3">
                      <div className="flex items-center justify-between gap-2">
                      ...
                      {evaluation.corrections?.length > 0 && (<div className="text-sm space-y-1.5">
                      ...
                      {evaluation.explanations?.grammarPoints && (<div className="text-sm space-y-1.5">
                      ...
                      {evaluation.explanations?.cultureTips && (<div className="text-sm space-y-1.5">
                          <div className="text-xs font-semibold text-slate-600">文化提示</div>
                          <ul className="list-disc ml-5">
                            {evaluation.explanations.cultureTips.map((t, i) => (<li key={i}>{t}</li>))}
                          </ul>
                        </div>)}
                      {evaluation.suggestedExamples?.length > 0 && (<div className="text-sm space-y-1.5">
                          <div className="text-xs font-semibold text-slate-600">
                            同场景更多例句（都包含目标词）
                          </div>
                          <ul className="space-y-1">
                            {evaluation.suggestedExamples.map((t, i) => (<li key={i} className="flex items-start justify-between gap-2">
                                <span className="flex-1">{t}</span>
                                <button className="shrink-0 px-2 py-0.5 rounded-full border text-[10px] text-slate-700 hover:bg-slate-50 disabled:opacity-50" onClick={() => saveSentenceToDb(t, 'EVAL')} disabled={savingSentence === t} title="保存到该生词的句子库">
                                  {savingSentence === t ? '保存中…' : '保存'}
                                </button>
                              </li>))}
                          </ul>
                        </div>)}
                    </div>) : (<div className="text-xs text-slate-400">还没有研究结果，先在左侧造一句并点击“开始研究”。</div>)) : savedSentences.length > 0 ? (<ul className="text-sm space-y-1.5 max-h-64 overflow-auto pr-1">
                    {savedSentences.map((it) => (<li key={it.id} className="flex items-start justify-between gap-2">
                        ...
                        <button className="shrink-0 px-2 py-0.5 rounded-full border text-[10px] text-rose-700 hover:bg-rose-50" onClick={async () => {
                    const ok = confirm('确定删除这句吗？');
                    if (!ok)
                        return;
                    const res = await fetch(`/api/sentence-builder/saved/${encodeURIComponent(it.id)}`, {
                        method: 'DELETE',
                    });
                    if (res.ok) {
                        const w = (requiredWord || word).trim();
                  ...
                  </ul>) : (<div className="text-xs text-slate-400">
...
function SceneBuilderPage() {
    return (<react_1.Suspense fallback={<div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 py-8 px-4">
        <div className="max-w-6xl mx-auto">
