    ...
    return (<div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex min-h-screen w-full max-w-3xl flex-col items-center justify-center gap-12 py-32 px-16 bg-white dark:bg-black">
        ...
        <div className="flex flex-col items-center gap-4 w-full max-w-sm p-6 border rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
          ...
          <button onClick={checkHealth} disabled={loading} className="w-full h-12 bg-black dark:bg-white text-white dark:text-black rounded-full font-medium transition-transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50">
            {loading ? '检查中...' : '开始测试'}
          </button>
          
          {healthStatus && (<div className={`mt-4 w-full p-4 rounded-xl text-sm font-mono overflow-auto ${healthStatus.status === 'ok' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
